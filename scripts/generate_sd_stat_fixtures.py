...
Grain (health supplemental outreach):
  pit_key = f"{customer_id}||{outreach_batch_id}"  (same key used across SD and all STAT files)
...
Outputs (under databricks/proto/fixtures/ by default):
  ...
  stat_promotion.parquet      — STAT: supplemental outreach / benefit-band attributes
...
def main() -> None:
    ...
    customer_ids = np.array([f"MBR-{i + 1:08d}" for i in range(n)], dtype=object)
    outreach_pool = np.array([f"OUTREACH-AI-2026-{k:04d}" for k in range(120)], dtype=object)
    outreach_batch_ids = rng.choice(outreach_pool, size=n, replace=True)
    pit_keys = np.array([f"{c}||{p}" for c, p in zip(customer_ids, outreach_batch_ids)], dtype=object)
    ...
    # --- SD (score driver): one row per (member, supplemental outreach batch) touch ---
    response_yes = rng.random(n) < 0.035
    sd = pl.DataFrame(
        {
            "pit_key": pit_keys,
            "customer_id": customer_ids,
            "outreach_batch_id": outreach_batch_ids,
            "product_line": product_line,
            "outreach_channel": rng.choice(
                np.array(["Email", "Patient portal", "SMS", "Direct mail"]), size=n
            ),
            ...
            "household_income_index": rng.lognormal(mean=11.0, sigma=0.5, size=n).astype(np.float64),
            "monthly_supplemental_premium": rng.gamma(shape=2.2, scale=35.0, size=n).astype(np.float64),
            "months_since_last_benefit_event": rng.integers(0, 60, size=n, endpoint=True).astype(np.float64),
            "member_engagement_score": rng.lognormal(mean=8.4, sigma=0.45, size=n).astype(np.float64),
        }
    )
    ...
    promo = pl.DataFrame(
        {
            "pit_key": pit_keys,
            "outreach_touch_count_90d": rng.integers(0, 8, size=n, endpoint=True),
            "last_outreach_channel": rng.choice(
                np.array(["Email", "SMS", "Care navigator", None]), size=n, p=[0.35, 0.3, 0.25, 0.1]
            ),
            "benefit_tier_band": rng.choice(np.array(["BRONZE", "SILVER", "GOLD", "PLATINUM"]), size=n),
        }
    )
    ...
    member = pl.DataFrame(
        {
            "pit_key": pit_keys,
            "member_tier": rng.choice(np.array(["ESSENTIAL", "EXTENDED", "COMPREHENSIVE"]), size=n),
            "tenure_months": rng.integers(1, 240, size=n, endpoint=True),
            "active_coverages_ct": rng.integers(1, 5, size=n, endpoint=True),
        }
    )
    ...
    responders = pl.DataFrame(
        {
            ...
            "outreach_batch_id": outreach_batch_ids[pos_idx],
            "product_line": product_line[pos_idx],
            "responded_at": rng.integers(20260101, 20260630, size=len(pos_idx)),
            "response_channel": rng.choice(np.array(["Email", "Member portal", "Benefits line"]), size=len(pos_idx)),
        }
    )
    ...
    meta.write_text(
        ...
        "Join key: pit_key = customer_id || outreach_batch_id (string).\n"
